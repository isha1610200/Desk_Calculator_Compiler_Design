...
std::string  sequence_token_val[100];
std::string sequence_token_class[100];
  ...
  int previous_i = 0;
   for(int i=0;i< input.length();i++)
   {
   	  ...
   	  { 
   	  	if(i - previous_i > 0)
   	  	{
   	  		sequence_token_val[counter] = input.substr(previous_i,i-previous_i);
   	  	   sequence_token_class[counter] = "num";
   	  	   counter++;
   	  	}
   	  	 
   	  	 sequence_token_val[counter] = "+";
   	  	 sequence_token_class[counter] = "operator";
   	  	 counter++;
   	  	 previous_i = i+1;	
   	  }
   	  else if(input[i] == '-')
   	  ...
   	  else if(input[i] == 'n')
   	  {
   	  		if(i - previous_i > 0)
   	  	{
   	  	 ...
   	  	 sequence_token_val[counter] = "n";
   	  	 sequence_token_class[counter] = "enter";
   	  	 counter++;
   	  	 previous_i = i+1;	
   	  }
   	  else if(input[i] == '$')
   	  {	
   	  	if(i - previous_i > 0)
   	  	{
   	  	 ...
   	  	 sequence_token_val[counter] = "$";
   	  	 sequence_token_class[counter] = "input_end_marker";
   	  	 counter++;
   	  	 previous_i = i+1;	
   	  ...
   	  {	
   	  	...
   	  	if(i - previous_i > 0)
   	  	{
   	  	 ...
   	  	 sequence_token_val[counter] = "(";
   	  	 sequence_token_class[counter] = "bracket";
   	  	 counter++;
   	  	 previous_i = i+1;	
...
{		
		curr_pt += 1;
		current_token.token_val = sequence_token_val[curr_pt];
		current_token.token_class = sequence_token_class[curr_pt];
...
void Shift(std::string action_to_be_done,Stack<int>& Automaton_States, Stack<std::string>& Stack_Grammar_Symbols,float Calculator_Computation_Array[])
{
	...
	std::cout << " Action: Shifting token value:: " << current_token.token_val << " And going to state: " << new_state << std::endl;
...
int main()
{
	...
	InitializeDeskCalculator();
		//std::cout << LR_ParseTable[0].num << std::endl;

		//user input for checking-> can fill sequence_token_val etc...  LATER--> Use Lex
   ...
   std::cout << "             It prints  the parsing steps in detail       " << std::endl << std::endl;
